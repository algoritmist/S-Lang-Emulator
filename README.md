...
## Модель процессора
Процессор работает по 6-стадийному конвееру:
1. ```Fetch``` &ndash; загрузка инструкции из памяти
2. ```Decode``` &ndash; декодирование инструкции. На этом этапе также устанавливаются управляющие сигналы для ```ControlUnit```
3. ```Execute``` &ndash; выполнение инструкции (запись результата во временный регистр, установка флагов ```NZ```)
4. ```Access Memory 1``` &ndash; чтение/запись памяти данных, вычисление следующего значения ```PC```
5. ```Access Memory 2``` &ndash; чтение/запись из устройства ввода-вывода
6. ```Writeback``` &ndash; запись результата из временного регистра либо памяти в регистр
### DataPath
![DataPath](images/datapath.png)
#### Компоненты DataPath
```RegFile``` &ndash; регистровый файл. Имеет:
* 3 адресуемых порта чтения, адрес в ```rs1, rs2, rd```, результат в ```rd1, rd2, rd3```
* 1 адресуемый порт записи ```rd```, для записи установить ```WR = 1``` и ждать следующего такта
* Данные для записи выбираются мультиплексором ```selWrSrc```

```Instruction Memory```:
* 1 адресуемый порт чтения ```offset``` и результат ```instr```

```Data Memory```:
* 1 адресуемый порт чтения, ```offset``` &ndash; адрес, ```RD4``` результат
* 1 порт для разрешения записи ```WM``` и 1 порт для данных ```Data```

```IO Dev Memory```:
* 1 порт на разрешение чтения и 1 порт на разрешение записи (```RIO, WIO``` соответственно)
* 1 порт для данных ```Data```
* ```RD5``` &ndash; результат чтения

```Datapath``` передает ```ControlUnit``` флаги ```NZ``` и текущую инструкцию из памяти ```Instruction Memory```
